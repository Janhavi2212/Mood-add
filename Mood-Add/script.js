document.addEventListener('DOMContentLoaded', () => {
    ...
    function showMessage(text, isError = true) {
        ...
        if (text.toLowerCase().includes("analyzing")) { 
        } else {
    ...
    async function getActualMoodFromBackend(base64FullDataUrl) {
        const pureBase64 = base64FullDataUrl.split(',')[1];
        const backendUrl = 'http://localhost:3001/api/detect-mood'; 

        showMessage("Analyzing your mood with AI...", false); 
        ...
        try {
            const response = await fetch(backendUrl, {
                ...
                body: JSON.stringify({ imageDataB64: pureBase64 }),
            });
            ...
            if (!response.ok) {
                ...
                showMessage(`Mood detection failed: ${errorData.error || response.statusText}`, true);
                return "Neutral";
            }

            const data = await response.json();
            console.log("Mood received from backend:", data.mood);
            return data.mood || "Neutral"; 

        } catch (error) {
            hideMessage(); 
            console.error('Network error or failed to fetch from backend:', error);
            showMessage("Could not connect to mood service. Please try again.", true);
            return "Neutral"; 
        }
    }
    ...
    function resolveMood(detectedMood) {
            if (availableMoods.some(m => m.toLowerCase() === detectedMood.toLowerCase())) {
            return availableMoods.find(m => m.toLowerCase() === detectedMood.toLowerCase());
        }
        console.warn(`Received mood "${detectedMood}" not in available list or invalid. Defaulting to Neutral.`);
        return "Neutral";
    }
    ...
    function displayRecommendation(mood) {
        ...
        if (movie) {
            ...
            console.error("No movie found for mood (after resolve):", mood, ". Defaulting to Neutral movie.");
            const neutralMovie = movieDatabase["Neutral"];
            recommendationIntro.innerHTML = `We detected a <span class="detected-mood-highlight">Neutral</span> mood. Here's a great choice:`;
            if (neutralMovie) {
    ...
    checkMoodBtn.addEventListener('click', async () => {
        ...
        const detectedMoodFromApi = await getActualMoodFromBackend(imageDataUrl);
        
        const finalMood = resolveMood(detectedMoodFromApi); 
        setTimeout(() => {
